									 ...
									 bool o_skip_only)             ///< True if we only need to skip rules and not act on the data stack
{
	...
	TIXML_STRING S_temp;
	TIXML_STRING S_name;
	expression_result ** erpp_arg = NULL;
	unsigned u_arg;
	bool o_error;
...
void xpath_processor::v_function_false (
										unsigned u_nb_arg,               ///< Nb of arguments
										expression_result **)   ///< Argument list
{
	if (u_nb_arg)
...
void xpath_processor::v_function_equal_2_node (expression_result * , expression_result * )
{
	throw error_not_yet ();
}
...
void xpath_processor::v_order_recurs (TiXmlNode * Np_base, int & i_current)
{
	...
	Np_base -> SetUserData ((void *)(size_t)i_current++);
